...
def rptapp_substitute(a_fine, b_fine, c_fine, d_fine, x_coarse, M):
    N = len(a_fine)
    num_partitions = N / M
    x_fine = [None] * N
...
def rptapp(a_fine, b_fine, c_fine, d_fine, M, N_tilde, threshold=0, level=0):
    N_fine = len(a_fine)
    ...
    rptapp_reduce(a_fine, b_fine, c_fine, d_fine,
                  a_coarse, b_coarse, c_coarse, d_coarse, M)
    ...
    for i in range(0, N_fine//M):
        ...
        mtx_part = mtx[i_begin:i_end, i_begin:i_end]
        mtx_part_cond = np.linalg.cond(mtx_part)
        mtx_part_shape = np.shape(mtx_part)
        conds.append(mtx_part_cond)

        print("Partition {} (A_PP, size {}x{}), condition {:e}".format(i, mtx_part_shape[0], mtx_part_shape[1], mtx_part_cond))
    ...
    # Otherwise, do recursive call
        ...
        x_coarse = rptapp(a_coarse, b_coarse, c_coarse, d_coarse,
                          M, N_tilde, threshold, level=level+1)    

    # Substitute into fine system
    x_fine_rptapp = rptapp_substitute(a_fine, b_fine, c_fine, d_fine, x_coarse, M)
...
for M in Ms:
    ...
    # Solve coarse system (size <= N_tilde)
    # x_coarse = np.linalg.solve(mtx_coarse, d_coarse)
    #print("xcoarse:\n", x_coarse)
    #print("dcoarse:\n", d_coarse)
    ...
    x_fine_rptapp = rptapp(a_fine, b_fine, c_fine, d_fine, M, N_tilde)
