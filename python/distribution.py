...
def run_trials(mtx, a_fine, b_fine, c_fine, part, label, gen_samples, 
    ...
    trials_fre = [None]*n_trials
    trials_res = [None]*n_trials
    N_fine = len(a_fine)

    for k in range(0, n_trials):
        # TODO: when using the relative residual, generate the right-hand sides from a normal/
        # uniform distribution instead of the solutions x
        ...
        trials_res[k] = res_new
        trials_fre[k] = fre_new

    return trials_fre, trials_res
...
def main():
    ...
    trials_fre_static, trials_res_static = run_trials(
    ...
    trials_fre_static_cond, trials_res_static_cond = run_trials(
        mtx, a_fine, b_fine, c_fine, min_cond_static_part, "static + cond", gen_samples, 
        args.n_trials, args.pivoting, args.symmetric)

    trials_fre_static_res, trials_res_static_res = run_trials(
    ...
    trials_fre_rand_cond_unif, trials_res_rand_cond_unif = run_trials(
    ...
    trials_fre_rand_cond_norm, trials_res_rand_cond_norm = run_trials(
        mtx, a_fine, b_fine, c_fine, min_cond_rand_norm_part, "random_norm + cond", gen_samples, 
        args.n_trials, args.pivoting, args.symmetric)

    # trials_fre_rand_res_norm, trials_res_rand_res_norm = run_trials(
    ...
    trials_fre_reduce_cond, trials_res_reduce_cond = run_trials(
        mtx, a_fine, b_fine, c_fine, min_cond_reduce_part, "reduce + cond", gen_samples, 
        args.n_trials, args.pivoting, args.symmetric)

    # trials_fre_reduce_res, trials_res_reduce_res = run_trials(
    ...
    trials_fre_d = {
        'rand_min_cond_unif' : trials_fre_rand_cond_unif,
        # 'rand_min_cond_unif'  : trials_rand_res_unif,
        'rand_min_cond_norm' : trials_fre_rand_cond_norm,
        # 'rand_min_res_norm'  : trials_rand_res_norm,
        'reduce_min_cond'    : trials_fre_reduce_cond,
        # 'reduce_min_res'     : trials_fre_reduce_res,
        'static_min_cond'    : trials_fre_static_cond,
        'static_min_res'     : trials_fre_static_res,
        'static_M32'         : trials_fre_static
    }

    trials_res_d = {
        'rand_min_cond_unif' : trials_res_rand_cond_unif,
        # 'rand_min_res_unif'  : trials_rand_res_unif,
        'rand_min_cond_norm' : trials_res_rand_cond_norm,
        # 'rand_min_res_norm'  : trials_rand_res_norm,
        'reduce_min_cond'    : trials_res_reduce_cond,
        # 'reduce_min_res'     : trials_res_reduce_res,
        'static_min_cond'    : trials_res_static_cond,
        'static_min_res'     : trials_res_static_res,
        'static_M32'         : trials_res_static
    }
    ...
    plot_ecdf(trials_fre_d, "ecdf_fre_{}".format(f_suffix))
    plot_ecdf(trials_res_d, "ecdf_res_{}".format(f_suffix))

    # Write trial data to file (JSON) for later use
    # TODO: write file after each series has completed
    with open('trials_fre_{}.json'.format(f_suffix), 'w') as outfile:
        json.dump(trials_fre_d, outfile)
    with open('trials_res_{}.json'.format(f_suffix), 'w') as outfile:
        json.dump(trials_res_d, outfile)
